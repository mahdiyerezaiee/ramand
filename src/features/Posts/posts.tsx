...
import {setPagintion} from "../Card/cardSlice";
import {Fragment, useEffect} from "react";
import Card from 'react-bootstrap/Card';
import {Button} from "react-bootstrap";
import {setShow} from "../Modal/modalSlice";
...
const Posts = ({pageNumber , pageSize}:Props) => {
    /** state **/

    const totalPosts = useSelector((state:any) => state.card.posts.data )
    const paginatedPost = paginate({totalPosts, pageNumber, pageSize});
    ...
    useEffect(()=>{
        dispatch(setPagintion(paginatedPost))

    },[pageNumber , pageSize , totalPosts])


    const handleModal = (id:number) => {
        dispatch(setShow({show : true , id : id}))

    }
    return(
      <div className="row">
          {filterQuery.map((post: any) => (
